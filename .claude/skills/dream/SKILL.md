...
1. **Retro overdue**: Read `.sounding/tooling-ledger-log.md` last `## R` header date (file is append-only/oldest-first — use `grep '^## R' | tail -1`, never `-m1`). If **>=7 days ago** (or file doesn't exist) → triggered.
