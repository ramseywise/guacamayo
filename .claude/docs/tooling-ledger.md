...
| 2026-07-20 | `/sanyi init` gains step 2: verify candidates against implementation (call sites, path resolution, evidence scope, bypass defaults, divergent reimplementations) BEFORE writing entries; interview-guide auto-draft table flagged as declaration-only. Also: v2 funnel names (`/synthesize`, `/reflect`) corrected to v3 (`/dream`) in global CLAUDE.md + MEMORY.md | playground SANYI.md v1 was drafted 2026-07-17 from CLAUDE.md declarations and was false on arrival — both guardrail pipelines had zero call sites, evidence line overstated coverage, and a default-off bypass flag existed. Took a full ratification session to discover. Contracts born unverified go permanently silent (review reports only NEW violations) | `absence:contract-entry-with-zero-callsites-at-init for 2 inits` | hypothesis — next test: librarian or atlas interview |
| 2026-07-19 | `/docs-check` skill (21st global): structural drift detection between human docs (README, DESIGN.md) and codebase. Diff mode (recent changes) or full audit. Wired into `/code-review level:2+` Step 5. `docs_drift_warn.sh` hook (PostToolUse, advisory): nudges when code files change near a stale README | No mechanism detected doc-vs-code drift; READMEs went stale silently after restructures (e.g. interview rounds) | `presence:docs-check-finding within 3 code-review-L2 runs` | hypothesis |
| 2026-07-20 | Plan-doc outcome metric NOT built; `ABANDONED` exists in `~/.claude/rules/docs.md` but is never applied — abandoned plans are silently dropped instead of relabelled | Across 61 plan docs, 37 carry a Status line and ZERO say ABANDONED (EXECUTED 26, COMPLETE 5, SUPERSEDED 2, IN PROGRESS 1). Any success ratio reads ~100% by construction and measures labelling discipline, not outcomes. Vocabulary also unnormalised (EXECUTED vs COMPLETE vs RESEARCH COMPLETE); only 29/61 filenames date-prefixed | `presence:abandoned-status-used within 3 abandoned plans` | hypothesis — convention fix must precede the metric |
