...
Extracted from ramseywise/librarian tests/unit/test_repo_economics.py @ aa3166e
(GUA-93); librarian keeps its copy until Phase 5. Imports retargeted to telemetry.*.
...
from telemetry import sessions
from telemetry.dashboard import (
    JULY_BOUNDARY,
    weekly_commits_by_repo,
    weekly_cost_by_repo,
)
from telemetry.factstore import ALL_COLUMNS, upsert
from telemetry.gitstore import upsert_commits
from telemetry.sessions import _session_repos
...
def _pin_workspace_root(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sessions, "WORKSPACE_ROOT", "/Users/wiseer/workspace")
