...
by existing `render_*_card(store)` functions in telemetry/dashboard.py
(authored for the `patch_*_card` in-place path). This issue wires those same
functions into `inject_regions()` via telemetry/__main__.py — no new
...
from telemetry.factstore import upsert
...
def test_roundtrip_inject_card_renderers(
    ...
    import telemetry.dashboard as dashboard_mod
