...
Provenance: ramseywise/librarian shared/log_config.py @ aa3166e (LIB-110 layout;
pre-110 home app/log_config.py). Vendored for GUA-93 — see telemetry/sessions.py
for the drift rationale.

Enforces "secrets never committed, never logged" — the logging clause, held by a
filter rather than convention.

Usage (per ~/.claude/refs/logging.md):

    from telemetry.log_config import get_logger, configure_logging
