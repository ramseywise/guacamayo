...
- [friction] Searching the path an issue body names, finding nothing, reporting "not started" — three times in one wake (#32, #18, #19). The path in the issue was stale; the work sat one directory over. A count against a non-existent path returns 0 and is indistinguishable from untouched work. Fix applied same session: /wake now resolves every path an issue names before counting against it (wake/SKILL.md §"Resolve issue claims before reporting state") — 2026-08-12
